...
avg(d_luft.p10) over (order by d_lubw.time_stamp desc RANGE BETWEEN '1 day' PRECEDING AND CURRENT ROW) as Luftdaten,
avg(d_lubw.p10) over (order by d_lubw.ts desc RANGE BETWEEN '1 day' PRECEDING AND CURRENT ROW) as LUBW
...
LUBW
from data
group by daily, data.Luftdaten, data.LUBW
ORDER BY daily asc